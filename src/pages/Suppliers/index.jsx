...
import SuppliersData from '@/db/suppliers.json';
...
const Index = () => {
    ...
    const pageData = PagesData.find(p => p.page_name === 'suppliers');
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            if (searchValue) {
                let searchedValue = SuppliersData.filter(f => f.name[lang].toLowerCase().includes(searchValue.toLowerCase()))
                setGalleryData(searchedValue);
            } else {
                setGalleryData(SuppliersData);
            }
    ...
    return (
        <motion.div
            ...
            <Helmet>
                <title>Golden Flowers Cash&Carry - Suppliers</title>
                <link rel="canonical" href="http://gfcc.ru/suppliers" />
            </Helmet>
